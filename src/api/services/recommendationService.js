...
const courseFields = [
  ...
  "provider_slug",
  "learn_content_pricing_currency",
  "slug",
  "partner_currency",
  "level",
  "pricing_type",
...
function formatQueryForCG(req) {
  let region =
    ...
    req.query &&
    req.query[
      "c697d2981bf416569a16cfbcdec1542b5398f3cc77d2b905819aa99c46ecf6f6"
    ]
      ? req.query[
          "c697d2981bf416569a16cfbcdec1542b5398f3cc77d2b905819aa99c46ecf6f6"
        ]
...
const getAllArticles = (articles_obj) => {
  ...
  if (learn_advices.length > 0)
    learn_advices.map((article) =>
      all_articles.push("LEARN_ADVICE_" + article.id)
    );
  if (learn_guides.length > 0)
    learn_guides.map((article) =>
      all_articles.push("LEARN_GUIDE_" + article.id)
    );
  if (career_guides.length > 0)
    career_guides.map((article) =>
      all_articles.push("CAREER_GUIDE_" + article.id)
    );
...
const getBaseCurrency = (result) => {
  return result.learn_content_pricing_currency
    ? result.learn_content_pricing_currency.iso_code
    : null;
};
...
const getCurrencyAmount = (amount, currencies, baseCurrency, userCurrency) => {
  ...
  if (!currency_b) {
    currency_b = currencies.find(
      (o) => o.iso_code === process.env.DEFAULT_CURRENCY
    );
  }
...
const formatImageResponse = (imageObject) => {
  ...
  if (imageObject.large) image = imageObject.large;
    ...
    if (imageObject.formats.large && imageObject.formats.large.url)
      image = imageObject.formats.large.url;
    else if (imageObject.formats.medium && imageObject.formats.medium.url)
      image = imageObject.formats.medium.url;
    else if (imageObject.formats.small && imageObject.formats.small.url)
      image = imageObject.formats.small.url;
    else if (imageObject.formats.thumbnail && imageObject.formats.thumbnail.url)
...
module.exports = class recommendationService {
  async popularSkillBasedRecommendation(req) {
    try {
      let cacheData = await RedisConnection.getValuesSync(
        "popularSkillBasedRecommendation"
      );
      if (cacheData.noCacheData != true) {
        cacheData = await getlistPriceFromEcom(
          cacheData,
          "learn_content",
          req.query["country"]
        );
        return {
          ...
          data: { list: cacheData, mlList: [], show: "logic", cacheHit: true },
        };
      }
    } catch (err) {
      console.log(err);
        ...
        should: [
          {
            bool: {
              must: [
                {
                  query_string: {
                    fields: ["title^4", "skills^3", "topics^2", "categories"],
    ...
    try {
      const result = await elasticService.search("learn-content", esQuery, {
        from: offset,
        size: limit,
        _source: courseFields,
      });
      if (result.hits && result.hits.length) {
        result.hits = await getlistPriceFromEcom(
          result.hits,
          "learn_content",
          req.query["country"]
        );
        ...
        for (const hit of result.hits) {
          const data = await this.generateCourseFinalResponse(
            hit._source,
            currency
          );
          courses.push(data);
        ...
        RedisConnection.set(
          "popularSkillBasedRecommendation",
          courses,
          process.env.CACHE_EXPIRE_COURSE_RECOMMENDATION || 60 * 15
        );
      }
      return {
        success: true,
        message: "list fetched successfully",
        data: { list: courses, mlList: [], show: "logic" },
      };
    } catch (err) {
      console.log(err);
      return {
  ...
  async popularGoalBasedRecommendation(req) {
    try {
      let cacheData = await RedisConnection.getValuesSync(
        "popularGoalBasedRecommendation"
      );
      if (cacheData.noCacheData != true) {
        cacheData = await getlistPriceFromEcom(
          cacheData,
    ...
    try {
      ...
      topTenGoalsPreferredRole = await RedisConnection.getValuesSync(
        "preferred-role"
      );
    } catch (err) {
              ...
              must: [
                {
                  query_string: {
                    fields: ["title^4", "skills^3", "topics^2", "categories"],
                    query: topTenGoalsPreferredRole
                      .join(" OR ")
                      .replace("/", "\\/"),
                  },
                },
  ...
  async getRelatedCourses(req) {
    try {
      ...
      const mLCourses = await this.getSimilarCoursesML(
        courseId,
        req.query["country"],
        currency,
        page,
      ...
      if (result && result.hits.length > 0) {
        ...
        for (let hit of result.hits) {
          let course = await this.generateCourseFinalResponse(
            hit._source,
            currency
          );
          courses.push(course);
  ...
  async getPopularCourses(req) {
    ...
    try {
      ...
      if (cachedData.noCacheData != true) {
        courses = cachedData;
        courses = await getlistPriceFromEcom(
          courses,
          "learn_content",
          req.query["country"]
        );
        ...
        switch (subType) {
          case "Trending":
            sort = [
              { "activity_count.last_x_days.trending_score": "desc" },
              { ratings: "desc" },
            ];
            break;
          default:
        ...
        if (result.hits) {
          ...
          for (const hit of result.hits) {
            var data = await this.generateCourseFinalResponse(
              hit._source,
              currency
            );
            courses.push(data);
          ...
          RedisConnection.expire(
            cacheKey,
            process.env.CACHE_EXPIRE_COURSE_RECOMMENDATION
          );
        }
      }
      let response = {
  ...
  async getSimilarCoursesML(courseId, country, currency, page = 1, limit = 6) {
    ...
    if (result && result.length) {
      ...
      for (const courseElasticData of result.slice(offset, offset + limit)) {
        const courseData = await this.generateCourseFinalResponse(
          courseElasticData._source,
          currency
        );
  ...
  async getMLUserCourseRecommendations(
    user_id,
    recommendationType,
    country,
    currency,
    ...
    let result = await mLService.getUserCourseRecommendations(
      user_id,
      recommendationType
    );
    let courses = [];
  ...
  async getTopPicksForYou(req, callback) {
    try {
      ...
      const mLCourses = await this.getMLUserCourseRecommendations(
        userId,
        profileType == "profile" ? "learn_profile" : "goals",
        req.query["country"],
        currency,
        page,
        limit
      ...
      const { skillsKeywords = [], workExpKeywords = [] } =
        await this.getUserProfileKeywords(userId);
      ...
      if (profileType == "profile" || !profileType) {
        ...
        if (skillsKeywords.length) {
          ...
          esQuery.bool.should[0].query_string.query =
            skillsKeywords.join(" OR ");
          let result = await elasticService.search("learn-content", esQuery, {
            from: offset,
            size: limitForSkills,
            _source: courseFields,
        ...
        if (workExpKeywords.length) {
          ...
          esQuery.bool.should[0].query_string.query =
            workExpKeywords.join(" OR ");
          let result = await elasticService.search("learn-content", esQuery, {
            from: offset,
            size: limitForWorkExp,
            _source: courseFields,
          });
          if (result.hits && result.hits.length) {
            result.hits = await getlistPriceFromEcom(
        ...
        profileType == "goal" ||
        (!profileType && courses.length < 1)
      ) {
        const { highPriorityKeywords, lowPriorityKeywords } =
          await this.getKeywordsFromUsersGoal(userId);
  ...
  async getRecentlyViewedArticles(req) {
    try {
      ...
      return {
        success: true,
        message: "list fetched successfully",
        data: { list: articles, mlList: [], show: "logic" },
      };
    } catch (error) {
      console.log(
        "Error occured while fetching recently viewed articles",
        error
      );
      return { success: false, message: "failed to fetch", data: { list: [] } };
    }
  ...
  async getRecentlySearchedArticles(req) {
    try {
      ...
      console.log(
        "Error occured while fetching recently searched articles",
        error
      );
      return { success: false, message: "failed to fetch", data: { list: [] } };
    }
  ...
  async getPeopleAreAlsoViewingArticles(req) {
    try {
      ...
      console.log(
        "Error occured while fetching people are also viewing articles",
        error
      );
      return { success: false, message: "failed to fetch", data: { list: [] } };
    }
  ...
  async getTopPicksForYouArticles(req) {
    try {
      ...
      console.log(
        "Error occured while fetching top picks for you articles",
        error
      );
      return { success: false, message: "failed to fetch", data: { list: [] } };
    }
  ...
  async getFeaturedArticles(req) {
    try {
      ...
      let result = null;
      let query = null;
      let cacheKey = `Featured_Articles-${pageType}-${category || "category"}-${
        sub_category || "sub_category"
      ...
      switch (pageType) {
        ...
        case "advicePage":
          ...
          result = await elasticService.search("blog_home_page", query, {
            from: 0,
            size: 1,
            _source: ["featured_articles"],
          });
          if (result.hits && result.hits.length) {
        ...
        case "sectionPage":
          query = {
            bool: {
              must: [{ term: { "default_display_label.keyword": section } }],
            },
          };
          result = await elasticService.search("section", query, {
            from: 0,
            size: 1,
            _source: ["all_featured_articles"],
          });
        ...
        case "categoryPage":
          let categoryResponse = await fetch(
            `${apiBackendUrl}/categories?default_display_label=${category}`
          );
        ...
        case "subCategoryPage":
          let subCategoryResponse = await fetch(
            `${apiBackendUrl}/sub-categories?default_display_label=${sub_category}`
          );
          ...
          break;
        case "topicPage":
          let topicResponse = await fetch(
            `${apiBackendUrl}/topics?default_display_label=${topic}`
          );
          if (topicResponse.ok) {
          ...
          break;
        case "learnPathPage":
          let learnPathResponse = await fetch(
            `${apiBackendUrl}/learning-path-landing-page`
          );
      ...
      if (featured_articles && featured_articles.length > 0) {
        ...
        esQuery.bool.must = [{ terms: { "id.keyword": featured_articles } }];

        let result = await elasticService.search("article", esQuery, {
          _source: articleFields,
        });
        if (result.hits && result.hits.length) {
          for (const hit of result.hits) {
            const data = await this.generateArticleFinalResponse(hit._source);
      ...
      if (articles && articles.length < maxArticles) {
        ...
        if (sub_category) {
          esQuery.bool.filter.push({
            term: {
              "article_sub_categories.keyword":
                decodeURIComponent(sub_category),
            },
          });
        }
        if (topic) {
        ...
        let result = await elasticService.search("article", esQuery, {
          from: 0,
          size: maxArticles - articles.length,
          sortObject: sort,
          _source: articleFields,
        });
        if (result.hits && result.hits.length) {
      ...
      RedisConnection.expire(
        cacheKey,
        process.env.CACHE_EXPIRE_ARTICLE_RECOMMENDATION
      );
  ...
  async getArticleAdvice(req) {
    try {
      ...
      let articles = [];
      let maxArticles = 8;
      let cacheKey = `Article_Advice-${pageType}-${category || "category"}-${
        sub_category || "sub_category"
      }-${topic || "topic"}`;
      let cachedData = await RedisConnection.getValuesSync(cacheKey);
      if (cachedData.noCacheData != true) {
      ...
      if (articles && articles.length < maxArticles) {
        ...
        if (
          fetaured_articles.data &&
          fetaured_articles.data.list &&
          fetaured_articles.data.list.length > 0
        ) {
          fetaured_articles.data.list.map((article) =>
            exclude_articles.push(article.id)
          );
        }
        ...
        if (pageType == "learnPathPage") {
          ...
          let sections = [];
          let sectoinResult = await elasticService.search(
            "section",
            sectionQuery,
            { _source: ["default_display_label"] }
          );
          ...
          while (count <= remaining_articles) {
            for (let section of sections) {
              if (count < remaining_articles) {
                article_per_section[section]
                  ? (article_per_section[section] += 1)
                  : (article_per_section[section] = 1);
              }
              count++;
            }
          ...
          for (const [key, value] of Object.entries(article_per_section)) {
            ...
            if (result.hits && result.hits.length) {
              for (const hit of result.hits) {
                const data = await this.generateArticleFinalResponse(
                  hit._source
                );
                articles.push(data);
              }
  ...
  async generateCourseFinalResponse(
    result,
    currency = process.env.DEFAULT_CURRENCY
  ) {
    const partnerCourseImage = await RedisConnection.getValuesSync(
      "partner-course-image-" + result.partner_slug
    );
    let desktop_course_image = null,
      mobile_course_image = null,
      partner_logo = null;
    if (partnerCourseImage.noCacheData != true) {
      ...
      const query = {
        bool: {
          must: [
            { exists: { field: "desktop_course_image" } },
            { term: { "slug.keyword": result.partner_slug } },
          ],
        },
      };

      const imageResult = await elasticService.search(
        "partner",
        query,
        { size: 2000 },
        ["mobile_course_image", "desktop_course_image", "logo"]
      );

      if (imageResult.hits && imageResult.hits.length > 0) {
        desktop_course_image = imageResult.hits[0]._source.desktop_course_image
          ? imageResult.hits[0]._source.desktop_course_image
          : desktop_course_image;
        mobile_course_image = imageResult.hits[0]._source.mobile_course_image
          ? imageResult.hits[0]._source.mobile_course_image
          : mobile_course_image;
      }
    }
    ...
    let data = {
      ...
      cover_image: desktop_course_image
        ? desktop_course_image
        : result.images
        ? formatImageResponse(result.images)
        : null,
      sidebar_listing_image: desktop_course_image
        ? desktop_course_image
        : result.listing_image
        ? formatImageResponse(result.listing_image)
        : result.images
        ? formatImageResponse(result.images)
        : null,
      card_image: desktop_course_image
        ? desktop_course_image
        : result.card_image
        ? formatImageResponse(result.card_image)
        : result.images
        ? formatImageResponse(result.images)
        : null,
      card_image_mobile: mobile_course_image
        ? mobile_course_image
        : result.card_image_mobile
        ? formatImageResponse(result.card_image_mobile)
        : result.images
        ? formatImageResponse(result.images)
        : null,
      course_details: {
        ...
        pricing: {
          display_price:
            typeof result.display_price != "undefined" &&
            result.display_price != null
      ...
      is_subscription: result.subscription_price
        ? result.subscription_price
        : false,
      buy_on_careervira: result.buy_on_careervira
        ? result.buy_on_careervira
        : false,
    ...
    if (result.pricing_type == "Paid") {
      ...
      if (result.coupons && result.coupons.length > 0) {
        for (let coupon of result.coupons)
          if (
            coupon.validity_end_date == null ||
            coupon.validity_start_date == null ||
            isDateInRange(coupon.validity_start_date, coupon.validity_end_date)
          )
            data.couponCount++;
      }
    }
    ...
    let partnerData = await PartnerService.getPartner({
      params: { slug: result.partner_slug },
      query: { currency: currency },
    });
    if (
      partnerData &&
      partnerData.buy_on_careervira &&
      data.course_details.instruction_type != "Instructor Paced"
    ) {
    ...
    if (data.pricing_details) {
      data.pricing_details.display_price =
        typeof result.display_price != "undefined" &&
        result.display_price != null
          ? result.display_price
          : true;
    ...
    const LEARN_CONTENT_POPULARITY_SCORE_THRESHOLD =
      await RedisConnection.getValuesSync("COURSE_POPULARITY_SCORE_THRESHOLD");

    data.isPopular = false;
    if (
      LEARN_CONTENT_POPULARITY_SCORE_THRESHOLD &&
      result.activity_count &&
      result.activity_count.all_time.popularity_score >
        parseInt(LEARN_CONTENT_POPULARITY_SCORE_THRESHOLD)
    ) {
      data.isPopular = true;
    }

    const LEARN_CONTENT_TRENDING_SCORE_THRESHOLD =
      await RedisConnection.getValuesSync("COURSE_TRENDING_SCORE_THRESHOLD");

    data.isTrending = false;
    if (
      LEARN_CONTENT_TRENDING_SCORE_THRESHOLD &&
      result.activity_count &&
      result.activity_count.last_x_days.trending_score >
        parseInt(LEARN_CONTENT_TRENDING_SCORE_THRESHOLD)
    ) {
  ...
  async getAuthor(id) {
    try {
      ...
      if (id) {
        const query = {
          bool: {
            filter: [{ term: { user_id: id } }],
          },
        };
        const result = await elasticService.search("author", query, {
          _source: ["first_name", "last_name", "slug", "image"],
        });
  ...
  async generateArticleFinalResponse(result) {
    try {
      ...
      if (result.co_authors && result.co_authors.length > 0) {
        for (let co_author of result.co_authors) {
          author.push({
            ...
            slug: co_author.slug,
            image: co_author.image
              ? formatImageResponse(co_author.image)
              : null,
          });
        }
      }
      if (result.partners && result.partners.length > 0) {
        result.partners = result.partners.filter(
      ...
      if (result.partners && result.partners.length > 0) {
        ...
        const partnerResult = await elasticService.search(
          "partner",
          partnerQuery,
          { _source: ["name", "slug"] },
          null
        ...
        if (partnerResult.total && partnerResult.total.value > 0) {
          for (let hit of partnerResult.hits) {
            partners.push({
              ...
              image: hit._source.cover_image
                ? formatImageResponse(hit._source.cover_image)
                : null,
            });
          }
        }
      ...
      let data = {
        ...
        cover_image: result.cover_image
          ? formatImageResponse(result.cover_image)
          : null,
        sidebar_listing_image: result.listing_image
          ? formatImageResponse(result.listing_image)
          : result.cover_image
          ? formatImageResponse(result.cover_image)
          : null,
        card_image: result.card_image
          ...
          ? formatImageResponse(result.cover_image)
          : null,
        card_image_mobile: result.card_image_mobile
          ? formatImageResponse(result.card_image_mobile)
          : result.cover_image
          ? formatImageResponse(result.cover_image)
          : null,
        ...
        created_by_role: result.created_by_role
          ? result.created_by_role
          : "author",
      ...
      const ARTICLE_POPULARITY_SCORE_THRESHOLD = parseInt(
        await RedisConnection.getValuesSync(
          "ARTICLE_POPULARITY_SCORE_THRESHOLD"
        )
      );

      data.isPopular = false;
      if (
        ARTICLE_POPULARITY_SCORE_THRESHOLD >= 0 &&
        result.activity_count &&
        result.activity_count.all_time.popularity_score >
          ARTICLE_POPULARITY_SCORE_THRESHOLD
      ) {
      ...
      const ARTICLE_TRENDING_SCORE_THRESHOLD = parseInt(
        await RedisConnection.getValuesSync("ARTICLE_TRENDING_SCORE_THRESHOLD")
      );

      data.isTrending = false;
      if (
        ARTICLE_TRENDING_SCORE_THRESHOLD >= 0 &&
        result.activity_count &&
        result.activity_count.last_x_days.trending_score >
          ARTICLE_TRENDING_SCORE_THRESHOLD
      ) {
        data.isTrending = true;
      }
      return data;
  ...
  async relatedLearnPaths(req) {
    try {
      ...
      let esQuery = {
        bool: {
          filter: [{ term: { "status.keyword": "approved" } }],
          must_not: {
            term: {
              _id: learnPathId,
            },
      ...
      let result = await elasticService.search("learn-path", esQuery, {
        from: offset,
        size: limit,
        _source: learnPathFields,
      });
      ...
      if (result && result.hits.length > 0) {
        ...
        for (let hit of result.hits) {
          let learnPath = await this.generateLearnPathFinalResponse(
            hit._source,
            currency
          );
      ...
      const response = {
        success: true,
        message: "list fetched successfully",
        data: { list: learnPaths, mlList: [], show: show },
      };
  ...
  async getPopularLearnPaths(req) {
    ...
    let cacheKey = `popular-learn-paths-${subType}-${category || ""}-${
      sub_category || ""
    }-${topic || ""}-${priceType || ""}-${page}-${limit}`;
    let cachedData = await RedisConnection.getValuesSync(cacheKey);

    if (cachedData.noCacheData != true) {
      cachedData = await getlistPriceFromEcom(
        cachedData,
        "learn_path",
        req.query["country"]
      );
      return {
        success: true,
        message: "list fetched successfully",
        data: { list: cachedData, mlList: [], show: "logic" },
      };
    }
    ...
    try {
      ...
      if (priceType && priceType == "Free") {
        esQuery.bool.filter.push({ term: { "pricing_type.keyword": "Free" } });
      }
      if (priceType && priceType == "Paid") {
        esQuery.bool.filter.push({ term: { "pricing_type.keyword": "Paid" } });
      }
      let sort = null;
      switch (subType) {
        case "Trending":
      ...
      if (result.hits) {
        ...
        for (const hit of result.hits) {
          var data = await this.generateLearnPathFinalResponse(
            hit._source,
            currency
          );
      ...
      RedisConnection.expire(
        cacheKey,
        process.env.CACHE_EXPIRE_LEARN_PATH_RECOMMENDATION
      );
      let response = {
        success: true,
        message: "list fetched successfully",
        data: { list: learnpaths, mlList: [], show: "logic" },
      };
  ...
  async getRelatedLearningPathForCourse(req) {
    ...
    try {
      ...
      if (cachedData.noCacheData != true) {
        ...
        learnpaths = await getlistPriceFromEcom(
          learnpaths,
          "learn_path",
          req.query["country"]
        );
        ...
        let courseData = await elasticService.search("learn-content", esQuery, {
          _source: ["topics", "sub_categories", "categories", "skills"],
        });
        ...
        if (courseData.hits) {
          for (const hit of courseData.hits) {
            ...
            if (
              hit._source.sub_categories &&
              hit._source.sub_categories.length > 0
            ) {
      ...
      console.log(
        "Error while processing data for related learnpath for course",
        error
      );
      return { success: false, message: "failed to fetch", data: { list: [] } };
    }
  ...
  async getLearnPathRecommendationForUser(req) {
    ...
    try {
      ...
      if (result.hits && result.hits.length) {
        ...
        for (const hit of result.hits) {
          const data = await this.generateLearnPathFinalResponse(
            hit._source,
            currency
          );
          learnPaths.push(data);
      ...
      console.log(
        "Error while processing data for learn Paths Recommendations",
        error
      );
      let response = {
        success: false,
  ...
  async getPopularArticles(req) {
    ...
    try {
      ...
      if (skill) {
        esQuery.bool.filter.push({ term: { "article_skills.keyword": skill } });
      }
      if (section) {
        esQuery.bool.filter.push({ term: { "section_name.keyword": section } });
      }
      let sort = null;
  ...
  async generateLearnPathFinalResponse(
    result,
    currency = process.env.DEFAULT_CURRENCY
  ) {
    let orderedLevels = [
      ...
      "All Level",
      "Others",
    ]; //TODO. ordering should be sorting while storing in elastic search.
    let data = {
      id: `LRN_PTH_${result.id}`,
      title: result.title,
      slug: result.slug,
      cover_images: result.images ? formatImageResponse(result.images) : null,
      sidebar_listing_image: result.card_image_mobile
        ? formatImageResponse(result.card_image_mobile)
        : result.images
        ? formatImageResponse(result.images)
        : null,
      ...
      card_image_mobile: result.card_image_mobile
        ? formatImageResponse(result.card_image_mobile)
        : result.cover_iimagesmage
        ? formatImageResponse(result.images)
        : null,
      levels: result.levels
        ? orderedLevels.filter((value) => result.levels.includes(value))
        : [],
      pricing: {
    ...
    //SET popular and trending keys
    const LEARN_PATH_POPULARITY_SCORE_THRESHOLD = parseInt(
      await RedisConnection.getValuesSync(
        "LEARN_PATH_POPULARITY_SCORE_THRESHOLD"
      )
    );

    data.isPopular = false;
    if (
      LEARN_PATH_POPULARITY_SCORE_THRESHOLD >= 0 &&
      result.activity_count &&
      result.activity_count.all_time.popularity_score >
        LEARN_PATH_POPULARITY_SCORE_THRESHOLD
    ) {
    ...
    const LEARN_PATH_TRENDING_SCORE_THRESHOLD = parseInt(
      await RedisConnection.getValuesSync("LEARN_PATH_TRENDING_SCORE_THRESHOLD")
    );

    data.isTrending = false;
    if (
      LEARN_PATH_TRENDING_SCORE_THRESHOLD >= 0 &&
      result.activity_count &&
      result.activity_count.last_x_days.trending_score >
        LEARN_PATH_TRENDING_SCORE_THRESHOLD
    ) {
      data.isTrending = true;
    }
  ...
  async getRecentlyViewedCourses(req) {
    ...
    try {
      ...
      if (result.hits) {
        ...
        for (const hit of result.hits) {
          let data = await this.generateCourseFinalResponse(
            hit._source,
            currency
          );
  ...
  async getUserLastSearch(req) {
    ...
    const existSearch = await models.user_meta.findOne({
      where: { userId: userId, key: "last_search" },
    });

    let suggestionList =
      existSearch != null && existSearch.value != ""
        ? JSON.parse(existSearch.value)
        : { "learn-path": [], "learn-content": [], provider: [], article: [] };
  ...
  async recentlySearchedCourses(req) {
    try {
      ...
      console.log(
        "Error occured while fetching recently searched courses : ",
        error
      );
      return { success: false, message: "failed to fetch", data: { list: [] } };
    }
  ...
  async peopleAreAlsoViewing(req) {
    try {
      ...
      console.log(
        "Error occured while fetching people Are Also Viewing : ",
        error
      );
      return { success: false, message: "failed to fetch", data: { list: [] } };
    }
  ...
  async enquiryBasedRecommendation(req) {
    try {
      ...
      const offset = (page - 1) * limit;
      const courseValues = await models.enquiry.findAll({
        where: { userId: userId },
      });
      if (!courseValues) {
      ...
      console.log(
        "Error occured while fetching enquiry based recommendation : ",
        error
      );
      return { success: false, message: "failed to fetch", data: { list: [] } };
    }
  ...
  async wishlistBasedRecommendation(req) {
    try {
      ...
      const offset = (page - 1) * limit;
      const courseValues = await models.user_meta.findAll({
        where: { userId: userId, key: "course_wishlist" },
      });
      if (!courseValues) {
        return { success: false, message: "No list", data: { list: [] } };
      }
      ...
      console.log(
        "Error occured while fetching wishlist based recommendation : ",
        error
      );
      return { success: false, message: "failed to fetch", data: { list: [] } };
    }
  ...
  async coursesRecommendationForUser(req) {
    ...
    try {
      ...
      let response = {
        success: true,
        message: "list fetched successfully",
        data: { list: learnContents, type: "logic" },
      };

      return response;
    } catch (error) {
      console.log(
        "Error while processing data for learn Content Recommendations",
        error
      );
      let response = {
        success: false,
  ...
  async relatedCoursesForLearnPath(req) {
    ...
    try {
      ...
      if (cachedData.noCacheData != true) {
        ...
        let learnPathData = await elasticService.search("learn-path", esQuery, {
          _source: ["topics", "sub_categories", "categories", "skills"],
        });
      ...
      console.log(
        "Error while processing data for related courses for learnpaths",
        error
      );
      return { success: false, message: "failed to fetch", data: { list: [] } };
    }
  ...
  async relatedCoursesForArticle(req) {
    ...
    try {
      ...
      if (cachedData.noCacheData != true) {
        ...
        let articleData = await elasticService.search("article", esQuery, {
          _source: [
            "article_topics",
        ...
        if (articleData.hits) {
          for (const hit of articleData.hits) {
            ...
            if (
              hit._source.article_sub_categories &&
              hit._source.article_sub_categories.length > 0
            ) {
              sub_categories = hit._source.article_sub_categories;
            }
            if (
              hit._source.article_topics &&
              hit._source.article_topics.length > 0
            ) {
              topics = hit._source.article_topics;
            }
            if (
              hit._source.article_skills &&
              hit._source.article_skills.length > 0
            ) {
  ...
  async getRelatedArticle(req) {
    try {
      ...
      if (cachedData.noCacheData != true) {
        ...
        //priority 1 category list
        let priorityList1 = [
          "article_skills.keyword",
          "article_topics.keyword",
          "article_sub_categories.keyword",
        ];
        let priorityList2 = [
          "author_id",
  ...
  async getRecommendationForArticle(req) {
    ...
    try {
      ...
      if (cachedData.noCacheData != true) {
        ...
        let currentArticleData = await elasticService.search(
          "article",
          esQuery,
          { _source: ["section_name", "category"] }
        );
        ...
        if (
          ...
          related_article.data.list &&
          related_article.data.list.length > 0
        ) {
          related_article.data.list.map((article) =>
            exclude_articles.push(article.id)
          );
        }
  ...
  async getRecommendationArticlesforCourse(req) {
    ...
    try {
      ...
      console.log(
        "Error while processing data for related articles for course",
        error
      );
      return { success: false, message: "failed to fetch", data: { list: [] } };
    }
  ...
  async getRelatedArticlesForLearnPath(req) {
    ...
    try {
      ...
      console.log(
        "Error while processing data for related articles for learnpath",
        error
      );
      return { success: false, message: "failed to fetch", data: { list: [] } };
    }
  ...
  async articleRecommendationForUser(req) {
    ...
    try {
      ...
      console.log(
        "Error while processing data for article Recommendations",
        error
      );
      let response = {
        success: false,
  ...
  async jobTitleBasedRecommendation(req) {
    ...
    try {
      ...
      if (user_experiences.length > 0) {
        user_experiences.map((user_experience) =>
          jobTitles.push(user_experience.jobTitle)
        );
        ...
        let response = {
          success: true,
          message: "list fetched successfully",
          data: { list: learnContents, mlList: [], show: "logic" },
        };
  ...
  async getPopularComparison(req) {
    try {
        ...
        if (courseData.hits) {
          for (const hit of courseData.hits) {
            ...
            if (hit._source.topics && hit._source.topics.length > 0) {
              ...
              let result = await elasticService.search(
                "learn-content",
                esQuery,
                { from: 0, size: 10, sortObject: sort, _source: courseFields }
              );
            ...
            if (
              courses.length < 10 &&
              hit._source.categories &&
              hit._source.categories.length > 0
            ) {
              esQuery.bool.filter[0] = {
                terms: {
      ...
      for (let course of courses) {
        ...
        if (course.course_details.pricing.pricing_type == "FREE")
          course.default_price = 0;

        if (
          related_courses &&
          related_courses.data &&
          related_courses.data.list &&
          related_courses.data.list.length > 0
        ) {
          let final_course = null;
          let price_diff = null;
          for (let related_course of related_courses.data.list) {
            if (related_course.course_details.pricing.pricing_type == "FREE")
              related_course.default_price = 0;
            if (
              price_diff &&
              price_diff >
                Math.abs(related_course.default_price - course.default_price)
            ) {
              final_course = related_course;
              price_diff = Math.abs(
                related_course.default_price - course.default_price
              );
            } else if (!price_diff) {
  ...
  async getPopularProviders(req) {
    ...
    let providers = [];
    try {
      let cacheKey = `popular-providers-${subType}-${category || ""}-${
        program || ""
      }-${region || ""}-${page}-${limit}`;
      let cachedData = await RedisConnection.getValuesSync(cacheKey);
      if (cachedData.noCacheData != true) {
        ...
        let result = await elasticService.search("provider", esQuery, {
          from: offset,
          size: limit,
          sortObject: sort,
        });
      ...
      let response = {
        success: true,
        message: "list fetched successfully",
        data: { list: providers, mlList: [], show: "logic" },
      };
      return response;
    } catch (error) {
  ...
  async generateproviderFinalResponse(result) {
    ...
    if (result.ranks && result.ranks.length) {
      let ranks = [],
        rankings = await RedisConnection.getValuesSync(`rankings_slug_object`);
      let latestRankYear = await RedisConnection.getValuesSync(
        "provider_ranking_latest_year"
      );

      for (let item of result.ranks) {
        //send latest year rank only
        if (
          item.year == (latestRankYear[item.slug] || new Date().getFullYear())
        ) {
      ...
      } else if (ranks.length == 1) data.rank = ranks[0];
    }
      ...
      if (response.ok) {
        ...
        if (json) {
          for (let ranking of json) {
            ...
            if (ranking.image) {
              ranking_images[ranking.name]["iamge"] = formatImageResponse(
                ranking.image
              );
            }
            if (ranking.logo) {
              ranking_images[ranking.name]["logo"] = formatImageResponse(
                ranking.logo
              );
            }
          }
  ...
  async lgCourseRecommendationForTechinicalSkill(req) {
    ...
    let courses = [];
    try {
      let cacheKey = `lg-Techinical-Skill-courses-${articleId}-${
        skill || "skill"
      }-${region || "region"}--${noRegion || "noRegion"}-${page}-${limit}`;
      let cachedData = await RedisConnection.getValuesSync(cacheKey);
      if (cachedData.noCacheData != true) {
        courses = cachedData;
        courses = await getlistPriceFromEcom(
        ...
        let result = await elasticService.search("article", esQuery, {
          _source: ["categories", "article_sub_categories", "article_topics"],
        });
        if (result.hits && result.hits.length) {
          for (const hit of result.hits) {
        ...
        esQuery = {
          bool: {
            must: [{ term: { "status.keyword": "published" } }],
          },
        };
        if (category) {
          esQuery.bool.must.push({
        ...
        result = await elasticService.search("learn-content", esQuery, {
          from: offset,
          size: limit,
          sortObject: sort,
          _source: courseFields,
      ...
      console.log(
        "Error while processing data for lgCourseRecommendationForTechinicalSkill courses",
        error
      );
      let response = {
        success: false,
  ...
  async lgHowToLearncourses(req) {
    ...
    let courses = [];
    try {
      let cacheKey = `lg-how-to-learn-courses-${articleId}-${
        level || "level"
      }--${learnType || "learnType"}--${priceType || "priceType"}-${
        region || "region"
      }--${noRegion || "noRegion"}-${page}-${limit}`;
      let cachedData = await RedisConnection.getValuesSync(cacheKey);
      if (cachedData.noCacheData != true) {
        courses = cachedData;
        courses = await getlistPriceFromEcom(
        ...
        let result = await elasticService.search("article", esQuery, {
          _source: [
            ...
            "article_sub_categories",
            "article_topics",
            "technical_skills_advanced_skills",
            "technical_skills_beginner_skills",
            "technical_skills_intermediate_skills",
          ],
        });
        if (result.hits && result.hits.length) {
          for (const hit of result.hits) {
            ...
            if (level == "Beginner" || !level) {
              hit._source.technical_skills_beginner_skills.map((skill) =>
                skills.push(skill.default_display_label)
              );
            }
            if (level == "Intermediate" || !level) {
              hit._source.technical_skills_intermediate_skills.map((skill) =>
                skills.push(skill.default_display_label)
              );
            }
            if (level == "Advanced" || !level) {
              hit._source.technical_skills_advanced_skills.map((skill) =>
                skills.push(skill.default_display_label)
              );
            }
          }
        ...
        if (priceType && priceType == "Free") {
          esQuery.bool.must.push({ term: { "pricing_type.keyword": "Free" } });
        }
        if (priceType && priceType == "Paid") {
          esQuery.bool.must.push({ term: { "pricing_type.keyword": "Paid" } });
        }
        let sort = [
          { "activity_count.all_time.popularity_score": "desc" },
          { ratings: "desc" },
  ...
  async cgCoursesWithPlacement(req) {
    ...
    let courses = [];
    try {
      let cacheKey = `cg-courses-with-Placement-${articleId}-${
        region || "region"
      }--${noRegion || "noRegion"}--${page}-${limit}`;
      let cachedData = await RedisConnection.getValuesSync(cacheKey);
      if (cachedData.noCacheData != true) {
      ...
      console.log(
        "Error while processing data for cgCoursesWithPlacement",
        error
      );
      let response = {
        success: false,
  ...
  async cgCoursesWithLearnType(req) {
    ...
    try {
      ...
      console.log(
        "Error while processing data for cgCoursesWithLearnType",
        error
      );
      let response = {
        success: false,
  ...
  getUserProfileKeywords = async (userId) => {
    ...
    if (userData && userData.length > 0) {
      for (let data of userData) {
        for (let user_skill of data.user_skills) {
          if (user_skill.isPrimary) skillsKeywords.push(user_skill.skill);
        }
      }
    }
    ...
    const workExperience = await models.user_experience.findAll({
      attributes: ["jobTitle", "industry"],
      where: { userId: userId },
    });
  ...
  getKeywordsFromUsersGoal = async (userId) => {
    ...
    for (const goal of goals) {
      ...
      const goalSkills = await models.skill.findAll({
  ...
  async getRelatedTrendingList(req) {
    try {
      ...
      //priority 1 category list
      let priorityList1 = [
        "list_topic.keyword, list_sub_category.keyword",
        "skills.keyword",
        "list_topic.keyword",
      ];
      let priorityList2 = [
      ...
      let result = await elasticService.search("trending-list", esQuery, {
        from: offset,
        size: limit,
        _source: trendingListFields,
      });
      ...
      const response = {
        success: true,
        message: "list fetched successfully",
        data: { list: trendingLists, mlList: [], show: show },
      };

      return response;
    } catch (error) {
      console.log(
        "Error while processing data for related trending list",
        error
      );
      const response = {
        success: false,
