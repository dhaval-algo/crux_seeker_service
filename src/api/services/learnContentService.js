...
const getBaseCurrency = (result) => {
    return result.learn_path_pricing_currency? result.learn_path_pricing_currency.iso_code:null;
};
...
module.exports = class learnContentService {
    ...
    async generateSingleViewData(result, isList = false, currency=process.env.DEFAULT_CURRENCY, isCaching = false){
        ...
        let tax = 0.0;
        let canBuy = false;
        if(result.learn_path_pricing_currency && result.learn_path_pricing_currency.iso_code === "INR" && result.pricing_type !="Free") {
            canBuy = true;
        ...
        let data = {
            ...
            currency: result.learn_path_pricing_currency?result.learn_path_pricing_currency:null,            
            instructors: [],
